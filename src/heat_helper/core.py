...
def _parse_year_group_to_int(year_group: str | int | pd.Series) -> int | pd.Series:
    ...
    # Reject bool up front: bool is a subclass of int
    if isinstance(year_group, bool):
        raise TypeError(f"Input must be str or int, not {type(year_group).__name__}")

    # Normalise whole-number floats (e.g. 5.0 from Excel) to int
    if isinstance(year_group, float):
        if math.isnan(year_group):
            raise TypeError("Input must be str or int, not NaN")
        if not year_group.is_integer():
            raise InvalidYearGroupError(year_group)
        logger.debug("_parse_year_group_to_int: Float %s coerced to int", year_group)
        year_group = int(year_group)

    # Now dispatch on the (possibly converted) value
    if isinstance(year_group, str):
        ...
        logger.debug("_parse_year_group_to_int: Str %s coerced to %d", year_group, int_year_group)
