...
def get_updates(df: pd.DataFrame, new_col: str, heat_col: str) -> pd.Series:
    ...
    if left.dtype != right.dtype:
        print(f"WARNING: Type Mismatch: {new_col} is {left.dtype}, but {heat_col} is {right.dtype}")
...
def get_contextual_updates(df: pd.DataFrame, new_col: str, heat_col: str, bad_values: list[str]) -> pd.Series:
