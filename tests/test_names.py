import pytest
import pandas as pd
from heat_helper.names import format_name, find_numbers_in_text, remove_numbers_from_text, remove_diacritics, create_full_name
...
def test_format_names_not_str_raises_error():
    text = 12
    with pytest.raises(TypeError, match=f"Text must be a string, not {type(text).__name__}"):
        format_name(text)
...
@pytest.mark.parametrize("num_name, clean_num_name", [
    ("Jane Doe", False),    
    ("Jane O'Doe", False),
    ("Jane Doe 21", True),   
    ("1 Jane Doe 79", True),     
...
def test_find_numbers_error():
    with pytest.raises(TypeError, match=f"Text must be a string, not {type(12).__name__}"):
...
def test_find_numbers_ignore():
    assert find_numbers_in_text(123, errors='ignore') == "Input not string: result unknown."
...
def test_remove_numbers_ignore_errors():
    assert remove_numbers_from_text(12, errors='ignore') == "Input not string: can't remove numbers."

@pytest.mark.parametrize("num_name, remove_num_name", [
    ("Jane Doe", "Jane Doe"),    
